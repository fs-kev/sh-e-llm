...
def iniciar_traduccion ():
    ...
    print("Pega tu código aquí. Cuando termines, presiona Ctrl+D (en Mac/Linux) o Ctrl+Z y Enter (en Windows).")
    ...
    if not codigo_usuario.strip():
        print("[bold red]No se ha ingresado ningún código. Volviendo al menú principal.[/bold red]")
        return 
    
    print("\n[bold]Código recibido:[/bold]")
    codigo_resaltado = Syntax(codigo_usuario, "python", theme="dracula", line_numbers=True)
    ...
    print(Panel(Markdown(explicacion, style="dracula"), title="[green]Análisis del Código[/green]", border_style="green"))
